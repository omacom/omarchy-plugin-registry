...
- Up to four root screenshots, `preview1` through `preview4`, each with one `.png`, `.jpg`, `.jpeg`, `.webp` or `.gif` extension. Legacy `preview` is an alias for slot 1, not a fifth image; duplicate slots/formats and other numeric suffixes are rejected. Gaps are allowed and the lowest numbered screenshot is the cover. Each image is capped at 10 MiB, within the unchanged package size limits. All screenshots are validated in the isolated media processor. The public gallery and JSON `screenshots` array follow numeric order; the existing `preview` API remains the cover. These rules apply to plugins too.
- Images named `preview-unlock`, `lock`, `unlock` or `screensaver` with those extensions; wallpaper images directly under `backgrounds/`. These are theme assets, not gallery slots. The boot unlock picker uses `preview-unlock.png` and `unlock.png`; applying them remains a separate privileged boot configuration step, not part of theme installation.
...
Deploy the additive registry migration first, then roll out the companion client before announcing theme installation. Numbered theme screenshots also require the updated companion validator; older theme clients reject these new filenames. The gallery uses existing Active Storage tables and needs no additional migration. Existing single-preview releases work without republishing. Existing plugin endpoints and signing identities remain unchanged. Do not silently relocate dirty directories, relabel Git installs as reviewed releases, or rename a registry origin without a coordinated pin/token migration.
